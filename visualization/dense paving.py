...
class LogoPainting:
    def __init__(self, logo_path, num_logos=1000, single_logo_size=None):
        ...
        if single_logo_size is None:
            single_logo_size=[150,150]
        else:
            self.single_logo_size = single_logo_size

        logos_per_dimension = math.ceil(math.sqrt(self.num_logos))

        self.canvas_height = logos_per_dimension * self.single_logo_size[0]
        self.canvas_width = logos_per_dimension * self.single_logo_size[1]
    ...
    def paint(self):
        ...
        self.canvas.save(f"/workspaces/Palette/outputs/Dense-{self.num_logos}-logos.png")


if __name__ == "__main__":
    logo_painting = LogoPainting("/workspaces/Palette/images/uni logo", num_logos=1000, single_logo_size=[150,150])
    logo_painting.paint()